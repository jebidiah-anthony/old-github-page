...
tags: [hackthebox, htb, writeup, linux, smtp, sql-injection, caching, ajenti, ajenti-filesystem, ssh-keys, gnu-screen] 
---
...
```
__NOTE(S)__:
- A couple of __DNS__ were captured by __`nmap`__

---

## PART 2 : PORT ENUMERATION

- Add the domain names to __/etc/hosts__:
  ```
  127.0.0.1       localhost
  127.0.1.1       kali f
...
- Run `gobuster` but limit to `200` responses:
  ```console
...
- __`https://custoomercare.megabank.htb/shell.php`__:
  - Page Source:
    ```html
    <!--
    53cret 5shell
...
>   
- __`https://freeflujab.htb/?smtp_config`__:
  ```console
  ...
  ```
  __NOTE(S)__:
  - The page redirects to __`/?denied`__
  - Maybe the cookie needs to be set correctly to access the page
>
- __`https://freeflujab.htb/?book`__:
  - Booking Form:
...
>
- __`view-source:https://freeflujab.htb/?info`__:
  ```html
  ...omitted...
     ...
     5. Retrieve relevant information from the database:
        ...
        - SQL Injection payload:
          ...
          - `admin` table:
            ...
            created | 2018-07-02 08:45:19
            modified | 2018-12-02 00:01:02
            modifiedby | 
            password | a3e30cce47580888f1f185798aca22ff10be617f4a982d67643bb56448508602
            passwordchanged | 2018-07-02
            superuser | 1
            disabled | 0
            privileges | 
...
>
3. Accessing __`https://sysadmin-console-01.flujab.htb:8080`__:
   ...
   - Cached redirects could be bypassed by:
     ...
     - Adding a __*unique*__ parameter that hasn't been cached:
       - __`/?`__, __`/?something_unique`__, or __`/?unique=something`__
...
5. Revisit __`https://sysadmin-console-01.flujab.htb:8080/?`__:
   - Landing Page: __`/view/login/normal`__
...
>
   - __`/view/notepad//home/drno/.ssh/userkey`__:
     ```
...
8. Connect to 10.10.10.124 via `ssh`:
   ...
   1. Go back to Ajenti's Notepad:
      - __`/view/notepad//etc/ssh/sshd_config`__
        ```console
...
>
      - __`/view/notepad//home/drno/.ssh/authorized_keys`__:
        ```
...
>   
   2. Edit `hosts.allow` file using Ajenti's Notepad:
      - __`/view/notepad//etc/hosts.allow`__:
        ```
